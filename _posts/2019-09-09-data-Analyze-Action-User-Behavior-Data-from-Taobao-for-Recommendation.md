...
需要注意的是，**PV**和**UV**在一定程度上可以看作是**虚荣指标**。在《精益数据分析》中提到：
...
晚间从19点开始，用户各种行为开始上升，至21点左右达到一天的最高峰。这一时刻的PV是一天中最高值。但这种高浏览行为是否带来了更多的购买行为呢？计算buy行为与PV行为的比例，结果如下：

![MM1zTS.png](https://s2.ax1x.com/2019/11/11/MM1zTS.png)

buy/pv的最大值是在上午10点，而在21时，虽然PV行为达到了顶峰，但buy行为比例相对于上午反而下降。
...
根据结果对用户进行划分（缺少交易金额数据，划分不准确）：

|                | recentScore(1-2)               | recentScore(3-4) |
| -------------- | ------------------------------ | ---------------- |
| freqScore(1-2) | 重要挽回用户（或流失用户）     | 重要维护用户     |
| freqScore(3-4) | 重要唤回用户（假设交易金额大） | 重要价值用户     |

对结果进行筛选，可以发现用户ID为486458和866670的用户为重要价值用户。（**备注：**有一点觉得对交易频率的划分过于武断和严格）
...
- 与非付费用户相比，付费用户的点击贡献量更大，平均25.34次点击贡献一个商品。

- 针对非付费用户点击最多的商品，可以通过优惠券、红包、返利或是推送更具性价比的同类商品给用户等形式促进用户购买。

- 晚间从19点开始，用户各种行为开始上升，至21~22点左右达到一天的最高峰。

- 计算buy/pv，发现该值在上午10点最高，到晚上有高浏览行为时反而下降。据此，可开展更多促销。

- 针对RFM分析的结果，对不同得分的用户精细化营销

  - 对用户ID为486458等的重要价值客户，给予更多关注。
  - 针对重要维护用户，即购买日期近但购买次数少的用户，采取关联商品推荐等手段，促进用户购买行为。
  - 针对重要唤回用户，即购买次数多但上次购买时间较远的用户，要采取更多唤醒策略，防止用户进一步流失。
  - 针对重要挽回用户，即购买次数书，距上次购买日期也较远的用户，分析其流失原因，必要时可以通过问卷或用户访谈的行为进一步了解用户心理。

  ##### 总结

  - 拖得时间有点长
  - 对各模型和指标的运用不熟悉，可以进行更多的练习，深入了解其优缺点和适用范围
  - 看到真实的用户行为和心理学研究能够交叉印证，觉得特别好玩，有趣。
...
- 2019-10-27 22:16:55，初稿完成
- 2019-11-11 10:00:29，更新buy/pv
